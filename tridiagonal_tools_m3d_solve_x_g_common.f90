...
!   character(len=clen), save :: rcsid_tridiagonal_tools_m3d_solve_x_g_common = "$Id: tridiagonal_tools_m3d_solve_x_g_common.f90,v 1.4 2021/04/26 15:44:44 ps Exp ps $"
    ...
    select case (m3d_solver)
      case default
        write (out,"('tridiagonal_tools%m3d_solve_g_x: Unrecognized solver ',a)") trim(m3d_solver)
        stop 'tridiagonal_tools%m3d_solve_g_x: Bad solver choice'
      case ('unpivoted','cyclic')
        call m3u_solve_x(mf,r,x)
      case ('pivoted')
        call m3dp_solve_x(mf,lpiv,r,x)
      case ('refined')
        nrhs = size(r,dim=2)
        ...
        refine_iterations: do ipass=1,m3d_iterations
          call m3d_multiply_x(m,x,scr(:,1:nrhs))
          scr(:,1:nrhs) = scr(:,1:nrhs) - r(:,1:nrhs)
          call m3dp_solve_x(mf,lpiv,scr(:,1:nrhs),scr(:,nrhs+1:2*nrhs))
